...
const Message: React.FC<Props> = ({ message, isDM, channelId, onReply }) => {
  ...
  const handleEdit = async () => {
    ...
    try {
      if (isDM) {
        await api.patch(`/dms/${channelId}/messages/${message.id}`, { content: editContent });
      } else {
        ...
        <div className="w-10 mr-3 flex-shrink-0 mt-0.5">
          {!message.isGrouped ? (
            message.avatar ? (
              ...
              <div
                className="w-10 h-10 rounded-full bg-discord-accent flex items-center justify-center text-white font-bold text-sm cursor-pointer"
                onClick={() => setProfileUserId(message.user_id)}
              >
